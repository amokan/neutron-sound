void loop() { 
  ...
  noiseTable2[buh]= random(-32767,32767); //hypnotoad noise
  
  NT3Rate = (random(-7,8))-(noiseTable3[0]/4198);
...
//--------------------------------------------------------  
  ...
  if (gateTest > gateOld) {gate=1;} //gate gets set to zero after oscs reset.
  gateOld = gateTest;       
  ...
  if (gate){  //reset wave to zero phase on gate input.
